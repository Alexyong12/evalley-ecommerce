...
export async function POST(request) {
  ...
  if (code !== row.code) {
    await db.prepare(`UPDATE otps SET attempts = attempts + 1 WHERE identifier = ? AND purpose = 'register'`).run(identifier);
    return err('That code is not correct.');
  }

  const data = JSON.parse(row.payload);
  const info = await db.prepare(`INSERT INTO users (name, identifier, identifier_type, birthdate, password, verified)
    VALUES (?,?,?,?,?,1)`).run(data.name, data.identifier, data.type, data.birthdate || null, hashPassword(data.password));
  await db.prepare(`DELETE FROM otps WHERE identifier = ? AND purpose = 'register'`).run(identifier);
