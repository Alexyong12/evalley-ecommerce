...
export async function POST(request) {
  ...
  const address = db.prepare('SELECT * FROM addresses WHERE id = ? AND user_id = ?').get(b.shipping_address_id, user.id);
  ...
  const tx = db.transaction(() => {
    ...
    const ii = db.prepare('INSERT INTO order_items (order_id, product_id, name, variant, sku, price, qty) VALUES (?,?,?,?,?,?,?)');
    for (const it of items) {
      ii.run(orderId, it.product_id, it.name, it.variant_id ? `${it.v_size} / ${it.v_color}` : null,
        it.v_sku || it.sku, it.price, it.qty);
      if (it.variant_id) db.prepare('UPDATE variants SET stock = stock - ? WHERE id = ?').run(it.qty, it.variant_id);
      else db.prepare('UPDATE products SET stock = stock - ? WHERE id = ?').run(it.qty, it.product_id);
    }
