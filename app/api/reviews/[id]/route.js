...
export async function PATCH(request, { params }) {
  ...
  const review = await db.prepare('SELECT * FROM reviews WHERE id = ? AND user_id = ?').get(id, user.id);
  ...
  await db.prepare(`UPDATE reviews SET rating = ?, body = ?, status = 'Pending' WHERE id = ?`).run(rating, body, review.id);
...
export async function DELETE(request, { params }) {
  ...
  if (!user) return NextResponse.json({ error: 'Sign in first.' }, { status: 401 });
  const { id } = await params;
  await db.prepare('DELETE FROM reviews WHERE id = ? AND user_id = ?').run(id, user.id);
