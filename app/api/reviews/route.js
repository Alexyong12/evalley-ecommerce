...
export async function POST(request) {
  ...
  const product = db.prepare('SELECT id FROM products WHERE slug = ? OR id = ?').get(b.product_slug || '', b.product_id || 0);
  ...
  db.prepare(`INSERT INTO reviews (product_id, user_id, author, rating, body, status)
