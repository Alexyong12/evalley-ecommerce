...
export async function PATCH(request, { params }) {
  ...
  const existing = await db.prepare('SELECT * FROM addresses WHERE id = ? AND user_id = ?').get(id, user.id);
  ...
  await db.prepare(`UPDATE addresses SET label=?, recipient_name=?, phone=?, country_code=?, line1=?, line2=?, city=?, state=?, postal_code=?, is_default=?
    WHERE id = ?`)
