...
export async function GET(request, { params }) {
  ...
  const order = await db.prepare('SELECT number, total, payment_method, payment_status FROM orders WHERE pay_token = ?').get(token);
...
export async function POST(request, { params }) {
  ...
  const order = await db.prepare('SELECT * FROM orders WHERE pay_token = ?').get(token);
  if (!order) return NextResponse.json({ error: 'Payment not found.' }, { status: 404 });
  ...
  if (b.action === 'pay') {
    ...
    await db.prepare('INSERT INTO notifications (user_id, title, body, kind) VALUES (?,?,?,?)')
