...
async function cartPayload(cartKey) {
  const items = cartKey ? await db.prepare(`
...
export async function GET() {
  ...
  return NextResponse.json(await cartPayload(cartKey), { headers: { 'Cache-Control': 'no-store' } });
}
...
export async function POST(request) {
  ...
  const product = b.product_id
    ? await db.prepare('SELECT * FROM products WHERE id = ?').get(b.product_id)
    : await db.prepare('SELECT * FROM products WHERE slug = ?').get(b.slug);
  ...
  const existing = await db.prepare('SELECT * FROM cart_items WHERE cart_key = ? AND product_id = ? AND variant_id IS ?')
  ...
  if (existing) {
    await db.prepare('UPDATE cart_items SET qty = qty + ? WHERE id = ?').run(qty, existing.id);
  } else {
    await db.prepare('INSERT INTO cart_items (cart_key, product_id, variant_id, qty) VALUES (?,?,?,?)')
      .run(cartKey, product.id, variant ? variant.id : null, qty);
  }
  return NextResponse.json({ ok: true, message: 'Added to cart.', ...(await cartPayload(cartKey)) });
}

export async function PATCH(request) {
  const b = await request.json().catch(() => ({}));
  const cartKey = await getCartKey();
  const item = await db.prepare('SELECT ci.*, p.stock AS p_stock, v.stock AS v_stock FROM cart_items ci JOIN products p ON p.id=ci.product_id LEFT JOIN variants v ON v.id=ci.variant_id WHERE ci.id = ? AND ci.cart_key = ?').get(b.id, cartKey);
  ...
  await db.prepare('UPDATE cart_items SET qty = ? WHERE id = ?').run(qty, item.id);
  return NextResponse.json({ ok: true, ...(await cartPayload(cartKey)) });
}

export async function DELETE(request) {
  const b = await request.json().catch(() => ({}));
  const cartKey = await getCartKey();
  await db.prepare('DELETE FROM cart_items WHERE id = ? AND cart_key = ?').run(b.id, cartKey);
  return NextResponse.json({ ok: true, ...(await cartPayload(cartKey)) });
}
