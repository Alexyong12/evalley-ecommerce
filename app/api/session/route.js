...
export async function GET() {
  ...
  const cartCount = cartKey
    ? (await db.prepare('SELECT COALESCE(SUM(qty),0) c FROM cart_items WHERE cart_key = ?').get(cartKey)).c
    : 0;
  let wishlistCount = 0, unread = 0;
  if (user) {
    wishlistCount = (await db.prepare('SELECT COUNT(*) c FROM wishlist WHERE user_id = ?').get(user.id)).c;
    unread = (await db.prepare('SELECT COUNT(*) c FROM notifications WHERE user_id = ? AND read = 0').get(user.id)).c;
  }
