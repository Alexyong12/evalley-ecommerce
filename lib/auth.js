...
export async function getSessionUser() {
  ...
  if (!sid) return null;
  const row = await db.prepare(`SELECT u.* FROM sessions s JOIN users u ON u.id = s.user_id WHERE s.id = ?`).get(sid);
  return row || null;
}
...
export async function createSession(userId) {
  ...
  await db.prepare('INSERT INTO sessions (id, user_id) VALUES (?, ?)').run(sid, userId);
...
export async function mergeGuestCart(userId) {
  ...
  if (!ck) return;
  const guestItems = await db.prepare('SELECT * FROM cart_items WHERE cart_key = ?').all(ck);
  ...
  `);
  for (const it of guestItems) await upsert.run('u' + userId, it.product_id, it.variant_id, it.qty);
  await db.prepare('DELETE FROM cart_items WHERE cart_key = ?').run(ck);
}
